...
interface Staff {
  ...
  phone: string;
  role: Role;
  currentShift: string | null; // e.g. "Ca sáng 6:00–14:00"
...
const MOCK_STAFF: Staff[] = [
  {
    ...
    email: "nguyenthingocnga20.03@winmart.com",
    phone: "0374055377",
    role: "STAFF",
    currentShift: null,
    shiftStatus: "nghi-ca",
  ...
  },
  {
    id: 3,
    name: "Trần Văn Bình",
    email: "tranvanbinh20.03@winmart.com",
    phone: "0972616244",
    role: "STAFF",
    currentShift: null,
    shiftStatus: "nghi-ca",
...
function ShiftTag({ status }: { status: ShiftStatus }) {
  const map: Record<ShiftStatus, { label: string; cls: string }> = {
    "dang-lam":      { label: "Đang làm",      cls: "border-[#227D52] text-[#227D52] bg-[rgba(34,125,82,0.06)]" },
    "nghi-ca":       { label: "Nghỉ ca",        cls: "border-(--border-button) text-(--text-muted) bg-(--bg-base)" },
    "chua-check-in": { label: "Chưa check-in",  cls: "border-[#C07A2B] text-[#C07A2B] bg-[rgba(192,122,43,0.06)]" },
  };
  const { label, cls } = map[status];
  return (
    <span className={`inline-block border rounded-[3px] px-2 py-0.5 text-[11px] font-semibold whitespace-nowrap ${cls}`}>
      {label}
    </span>
  );
}
...
function CreateModal({ onClose, onSubmit }: {
  onClose: () => void;
  onSubmit: (s: Omit<Staff, "id" | "currentShift" | "shiftStatus" | "daysWorked" | "totalDays">) => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-[2px]">
      <div className="bg-white border-2 border-(--border-button) rounded-lg shadow-xl w-[420px]">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-(--border-button)">
          <div>
            <h2 className="text-[14px] font-bold text-(--text-primary)">TẠO TÀI KHOẢN</h2>
            <p className="text-[11px] text-(--text-muted) mt-0.5">Thêm nhân viên mới vào hệ thống</p>
          </div>
        ...
        <div className="px-5 py-4 space-y-3">
          <div>
            <label className="block text-[11px] font-semibold text-(--text-muted) uppercase mb-1">Họ tên *</label>
          ...
          <div>
            <label className="block text-[11px] font-semibold text-(--text-muted) uppercase mb-1">Email *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-(--text-muted) uppercase mb-1">Số điện thoại *</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-[11px] font-semibold text-(--text-muted) uppercase mb-1">Role</label>
              <div className="relative">
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-(--border-button)">
          ...
          <button onClick={handleSubmit} disabled={!name || !email || !phone}
...
export default function StaffPage() {
  ...
  const kpiDangLam     = staffList.filter(s => s.shiftStatus === "dang-lam").length;
  const kpiNghi        = staffList.filter(s => s.shiftStatus === "nghi-ca").length;
  const kpiChuaCheckIn = staffList.filter(s => s.shiftStatus === "chua-check-in").length;
  ...
  const data = useMemo(() => {
    return staffList.filter(s => {
      ...
      if (shiftFilter === "dang-lam" && s.shiftStatus !== "dang-lam") return false;
      if (shiftFilter === "nghi-ca" && s.shiftStatus !== "nghi-ca") return false;
      if (activeTab === "dang-lam" && s.shiftStatus !== "dang-lam") return false;
  ...
  function toggleAll() {
    if (allSelected) setSelected(prev => { const n = new Set(prev); data.forEach(s => n.delete(s.id)); return n; });
    else setSelected(prev => { const n = new Set(prev); data.forEach(s => n.add(s.id)); return n; });
  }
  ...
  // ── Add staff ──
  function handleCreate(s: Omit<Staff, "id" | "currentShift" | "shiftStatus" | "daysWorked" | "totalDays">) {
    setStaffList(prev => [...prev, {
      ...s, id: prev.length + 1,
      currentShift: null, shiftStatus: "nghi-ca", daysWorked: 0, totalDays: 26,
    }]);
  }
  ...
  return (
    <div className="p-6 space-y-6 bg-(--bg-base) min-h-screen">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        {/* Shift filter */}
        <div className="relative">
          <select value={shiftFilter} onChange={e => setShift(e.target.value)}
            className="appearance-none pl-3 pr-7 py-1.5 text-[13px] border-2 border-(--border-button) rounded text-(--text-primary) outline-none cursor-pointer">
        ...
        <table className="w-full border-collapse">
          <thead className="bg-(--bg-table)">
            <tr>
              ...
              <th className="px-3 py-2.5 text-left text-md font-semibold text-(--text-muted) uppercase tracking-wide">Nhân viên</th>
              <th className="px-3 py-2.5 text-left text-md font-semibold text-(--text-muted) uppercase tracking-wide">Role</th>
              <th className="px-3 py-2.5 text-left text-md font-semibold text-(--text-muted) uppercase tracking-wide">Ca hiện tại</th>
              <th className="px-3 py-2.5 text-left text-md font-semibold text-(--text-muted) uppercase tracking-wide">Trạng thái</th>
              <th className="px-3 py-2.5 text-left text-md font-semibold text-(--text-muted) uppercase tracking-wide">Ngày làm</th>
              <th className="px-3 py-2.5 text-left text-md font-semibold text-(--text-muted) uppercase tracking-wide">Điện thoại</th>
            </tr>
          </thead>
          <tbody>
            {data.map(staff => (
              <tr key={staff.id}
                ...
                <td className="px-3 py-3">
                  <input type="checkbox" checked={selected.has(staff.id)} onChange={() => toggleOne(staff.id)}
                    className="w-3.5 h-3.5 accent-[#C0392B] cursor-pointer" />
                </td>
                ...
                <td className="px-3 py-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[13px] text-(--text-primary) font-medium">{staff.name}</span>
                      <span className="text-[11px] text-(--text-muted)">{staff.email}</span>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-3 py-3">
                  {staff.currentShift ? (
                    <span className="text-[12px] text-(--text-secondary)">{staff.currentShift}</span>
                  ) : (
                ...
                <td className="px-3 py-3">
                  <span className="text-[13px] font-mono text-(--text-primary)">
                    {staff.daysWorked}
                    <span className="text-(--text-muted)"> / {staff.totalDays}</span>
                  </span>
                </td>

                {/* Phone */}
                <td className="px-3 py-3">
                  <span className="text-[12px] font-mono text-(--text-secondary)">{staff.phone}</span>
                </td>
              </tr>
            ))}
